...
up: docker-up
down: docker-down
show-password: show-initial-password
deploy: deploy
...
deploy:
	...
	ssh ${HOST} -p ${PORT} 'cd jenkins && docker-compose down --remove-orphans'
	ssh ${HOST} -p ${PORT} 'cd jenkins && docker-compose build --pull'
	ssh ${HOST} -p ${PORT} 'cd jenkins && docker-compose up -d'